...
const NATIONALITY_ISO = {
  Germany:"DE", Greece:"GR", Bulgaria:"BG", Italy:"IT", Ireland:"IE", France:"FR",
  Spain:"ES", Portugal:"PT", Netherlands:"NL", Belgium:"BE", Austria:"AT", Poland:"PL",
  Sweden:"SE", Denmark:"DK", Finland:"FI", Cyprus:"CY", Luxembourg:"LU",
  Switzerland:"CH", "Czech Republic":"CZ", Romania:"RO", Hungary:"HU", Croatia:"HR",
};

function isoFromNationality(n) {
  return NATIONALITY_ISO[n] ?? (n ? n.slice(0, 2).toUpperCase() : "??");
}
...
function Flag({ code, w = 20 }) {
  ...
  const f = {
    ...
    ES: <g><rect width="90" height="15" fill="#C60B1E"/><rect y="15" width="90" height="30" fill="#FFC400"/><rect y="45" width="90" height="15" fill="#C60B1E"/></g>,
    PT: <g><rect width="27" height="60" fill="#006600"/><rect x="27" width="63" height="60" fill="#FF0000"/></g>,
    NL: <g><rect width="90" height="20" fill="#AE1C28"/><rect y="20" width="90" height="20" fill="#fff"/><rect y="40" width="90" height="20" fill="#21468B"/></g>,
    AT: <g><rect width="90" height="20" fill="#ED2939"/><rect y="20" width="90" height="20" fill="#fff"/><rect y="40" width="90" height="20" fill="#ED2939"/></g>,
    PL: <g><rect width="90" height="30" fill="#fff"/><rect y="30" width="90" height="30" fill="#DC143C"/></g>,
    SE: <g><rect width="90" height="60" fill="#006AA7"/><rect x="24" width="16" height="60" fill="#FECC02"/><rect y="22" width="90" height="16" fill="#FECC02"/></g>,
  };
  if (f[code]) {
    return <svg viewBox="0 0 90 60" width={w} height={h} style={{ borderRadius:2, flexShrink:0, verticalAlign:"middle", border:"1px solid rgba(255,255,255,0.15)" }}>{f[code]}</svg>;
  }
  return (
    <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:w, height:h, borderRadius:3, background:C.navyLight, border:`1px solid ${C.border}`, fontSize:Math.max(7, Math.round(w * 0.42)), fontWeight:700, color:C.muted, letterSpacing:0, flexShrink:0, verticalAlign:"middle" }}>
      {code}
    </span>
  );
}
...
function mapCase(raw) {
  return {
    ...
    flagCode:    isoFromNationality(raw.nationality),
    asset:       raw.asset_type,
    value:       raw.asset_value_eur,
    docId:       raw.encrypted_doc_id,
    status:      raw.status,
    priority:    derivePriority(raw.asset_value_eur),
  };
}
...
export default function Dashboard() {
  ...
  const pendingCnt = pendingCases.length;
  const totalCnt = cases.length;

  const filtered = search
    ? pendingCases.filter(c => (c.name + c.id + c.asset).toLowerCase().includes(search.toLowerCase()))
    : pendingCases;
      ...
      <main style={{ maxWidth:1280, margin:"0 auto", padding:"2rem 2rem 4rem" }}>
        ...
        <div style={{ display:"flex", gap:10, marginTop:18 }}>
          <div style={{ padding:"10px 20px", borderRadius:10, background:`${C.gold}12`, border:`1px solid ${C.gold}44`, display:"flex", alignItems:"baseline", gap:8 }}>
            <span style={{ fontSize:18, fontWeight:800, color:C.gold }}>{doneCnt}/{totalCnt}</span><span style={{ fontSize:10.5, color:C.muted }}>done today</span>
          </div>
          <div style={{ padding:"10px 20px", borderRadius:10, background:`${C.amber}12`, border:`1px solid ${C.amber}44`, display:"flex", alignItems:"baseline", gap:8 }}>
            <span style={{ fontSize:18, fontWeight:800, color:C.amber }}>{pendingCnt}</span><span style={{ fontSize:10.5, color:C.muted }}>pending</span>
          </div>
        </div>
        ...
        {loading && (
          <div style={{ textAlign:"center", padding:"80px 0", color:C.muted, fontSize:13 }}>Loading cases…</div>
        )}
        {!loading && error && (
          <div style={{ textAlign:"center", padding:"80px 0", color:"#EF5350", fontSize:13 }}>Couldn't reach the server — is the backend running?</div>
        )}
        {!loading && !error && filtered.length === 0 && (
          <div style={{ textAlign:"center", padding:"80px 0", color:C.muted, fontSize:13 }}>No cases found.</div>
        )}
        {!loading && !error && filtered.length > 0 && (
